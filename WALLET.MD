...
│  6. Encrypt seed phrase: AES-256-GCM(key, seed) [optional]      │
...
│  2. Derive per-user key: HMAC(SERVER_SECRET, userId)            │
│  3. Add server encryption: AES-256-GCM(userKey, blob)           │
...
async function createWallet(password: string) {
  ...
  const response = await fetch('/wallet', {
    ...
    body: JSON.stringify({
      address,
      encryptedKey: packEncrypted(iv, encryptedKey),
      encryptedSeed: packEncrypted(seedIv, encryptedSeed),
      salt: btoa(String.fromCharCode(...salt)),
      iterations: 100000
    })
  });
  
  return { address, mnemonic }; // Show mnemonic to user once!
}

// Pack IV + ciphertext into base64
function packEncrypted(iv: Uint8Array, ciphertext: ArrayBuffer): string {
